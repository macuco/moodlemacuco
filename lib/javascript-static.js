...
/**
 * INEA - FUNCION AGREGADA TEMPORALMENTE PARA AGREGACION DE RFE POR AJAX
 * @autor Juan Manuel
 * JavaScript Document
...
/* INEA - url: URL al que se llamara, query: Parametros, req: Instancia HTTPRequest*/
...
/**
 * INEA - Funcion que debe de determinar si la peticion es POST o GET
...
/* INEA - Identifica que funcion se va a llamar y que tipo de respuesta tendra */
function doCallback(callback,item){
		eval(callback+'(item)');
}

/* INEA - Funcion prueba echo */
function prueba(url,query,callback,reqtype,getxml){
	alert(url+"  "+callback+"  "+reqtype);
}

/* FUNCIONES DE LLAMADO */
/* INEA - Manda a llamar al elemento id_number (rfe) */
function recibir(rfe) {
	...
	//document.getElementById("idnumber").value = rfe;
}
	
/* INEA - Genera el RFE o RFC con los elementos appaterno, apmaterno, nombre, fechanacimiento */
...
/* INEA - Carga la lista de estados o entidades */
...
/* INEA - Filtrar laz zonas por estado */
...
/* INEA - Asignar origen segun entidad, municipio y zona */
...
// INEA - funcion para mostrar/ocultar bloques en seccion de informes AMEVYT
