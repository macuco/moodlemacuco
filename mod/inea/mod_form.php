...
class mod_inea_mod_form extends moodleform_mod {
    function definition() {
        global $CFG, $DB;
        $mform =& $this->_form;
        ...
        if ($this->current->instance and $this->current->tobemigrated) {
            ...
            $resource_old = $DB->get_record('inea_old', array('oldid'=>$this->current->instance));
            $mform->addElement('static', 'warning', '', get_string('notmigrated', 'inea', $resource_old->type));
        ...
        if (count($options) == 1) {
            ...
            $mform->addElement('select', 'display', get_string('displayselect', 'inea'), $options);
            $mform->setDefault('display', $config->display);
            $mform->addHelpButton('display', 'displayselect', 'inea');
        }

        $mform->addElement('checkbox', 'showsize', get_string('showsize', 'inea'));
        $mform->setDefault('showsize', $config->showsize);
        $mform->addHelpButton('showsize', 'showsize', 'inea');
        $mform->addElement('checkbox', 'showtype', get_string('showtype', 'inea'));
        $mform->setDefault('showtype', $config->showtype);
        $mform->addHelpButton('showtype', 'showtype', 'inea');
        $mform->addElement('checkbox', 'showdate', get_string('showdate', 'inea'));
        $mform->setDefault('showdate', $config->showdate);
        $mform->addHelpButton('showdate', 'showdate', 'inea');

        if (array_key_exists(RESOURCELIB_DISPLAY_POPUP, $options)) {
            $mform->addElement('text', 'popupwidth', get_string('popupwidth', 'inea'), array('size'=>3));
            if (count($options) > 1) {
            ...
            $mform->addElement('text', 'popupheight', get_string('popupheight', 'inea'), array('size'=>3));
            if (count($options) > 1) {
        ...
        if (array_key_exists(RESOURCELIB_DISPLAY_AUTO, $options) or
          ...
          array_key_exists(RESOURCELIB_DISPLAY_FRAME, $options)) {
            $mform->addElement('checkbox', 'printintro', get_string('printintro', 'inea'));
        ...
        $mform->addElement('select', 'filterfiles', get_string('filterfiles', 'inea'), $options);
    ...
    function data_preprocessing(&$default_values) {
        if ($this->current->instance and !$this->current->tobemigrated) {
            ...
            file_prepare_draft_area($draftitemid, $this->context->id, 'mod_inea', 'content', 0, array('subdirs'=>true));
