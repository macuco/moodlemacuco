...
/**
 * Private INEA module utility functions
 *
 * @package    mod_inea
 ...
 * @return void
 */
function inea_redirect_if_migrated($oldid, $cmid) {
    global $DB, $CFG;

    if ($oldid) {
        $old = $DB->get_record('inea_old', array('oldid'=>$oldid));
    } else {
        $old = $DB->get_record('inea_old', array('cmid'=>$cmid));
    }
 ...
 */
function inea_display_embed($resource, $cm, $course, $file) {
    global $CFG, $PAGE, $OUTPUT;

    $clicktoopen = inea_get_clicktoopen($file, $resource->revision);
    ...
    inea_print_header($resource, $cm, $course);
    inea_print_heading($resource, $cm, $course);

    echo $code;

    inea_print_intro($resource, $cm, $course);
 ...
 * @return does not return
 */
function inea_display_frame($resource, $cm, $course, $file) {
 ...
 */
function inea_get_clicktoopen($file, $revision, $extra='') {
    global $CFG;

    $filename = $file->get_filename();
    $path = '/'.$file->get_contextid().'/mod_inea/content/'.$revision.$file->get_filepath().$file->get_filename();
    $fullurl = file_encode_url($CFG->wwwroot.'/pluginfile.php', $path, false);

    $string = get_string('clicktoopen2', 'inea', "<a href=\"$fullurl\" $extra>$filename</a>");
 ...
 * Internal function - create click to open text with link.
 */
function inea_get_clicktodownload($file, $revision) {
    global $CFG;
    ...
    $string = get_string('clicktodownload', 'inea', "<a href=\"$fullurl\">$filename</a>");
 ...
 * @return does not return
 */
function inea_print_workaround($resource, $cm, $course, $file) {
    global $CFG, $OUTPUT;

    inea_print_header($resource, $cm, $course);
    inea_print_heading($resource, $cm, $course, true);
    inea_print_intro($resource, $cm, $course, true);

    $resource->mainfile = $file->get_filename();
    echo '<div class="resourceworkaround">';
    switch (inea_get_final_display_type($resource)) {
        case RESOURCELIB_DISPLAY_POPUP:
            $path = '/'.$file->get_contextid().'/mod_inea/content/'.$resource->revision.$file->get_filepath().$file->get_filename();
            $fullurl = file_encode_url($CFG->wwwroot.'/pluginfile.php', $path, false);
            ...
            echo inea_get_clicktoopen($file, $resource->revision, $extra);
            break;
        ...
        case RESOURCELIB_DISPLAY_DOWNLOAD:
            echo inea_get_clicktodownload($file, $resource->revision);
            break;

        case RESOURCELIB_DISPLAY_OPEN:
        default:
            echo inea_get_clicktoopen($file, $resource->revision);
            break;
    }
    echo '</div>';
 ...
 * @return void
 */
function inea_print_header($resource, $cm, $course) {
    global $PAGE, $OUTPUT;
 ...
 * @return void
 */
function inea_print_heading($resource, $cm, $course, $notused = false) {
    global $OUTPUT;
...
/**
 * Gets details of the file to cache in course cache to be displayed using {@link inea_get_optional_details()}
 *
 * @param object $resource Resource table row (only property 'displayoptions' is used here)
 * @param object $cm Course-module table row
 * @return string Size and type or empty string if show options are not enabled
 */
function inea_get_file_details($resource, $cm) {
    $options = empty($resource->displayoptions) ? array() : @unserialize($resource->displayoptions);
    $filedetails = array();
 ...
 * @return string Size and type or empty string if show options are not enabled
 */
function inea_get_optional_details($resource, $cm) {
    global $DB;
    ...
    if (!empty($options['showsize']) || !empty($options['showtype']) || !empty($options['showdate'])) {
        if (!array_key_exists('filedetails', $options)) {
            $filedetails = inea_get_file_details($resource, $cm);
        } else {
        ...
        if (!empty($options['showdate']) && (!empty($filedetails['modifieddate']) || !empty($filedetails['uploadeddate']))) {
            if (!empty($filedetails['modifieddate'])) {
                $date = get_string('modifieddate', 'mod_inea', userdate($filedetails['modifieddate'],
                    get_string('strftimedatetimeshort', 'langconfig')));
            } else if (!empty($filedetails['uploadeddate'])) {
                $date = get_string('uploadeddate', 'mod_inea', userdate($filedetails['uploadeddate'],
                    get_string('strftimedatetimeshort', 'langconfig')));
            }
            $langstring .= 'date';
            $infodisplayed += 1;
        ...
        if ($infodisplayed > 1) {
            $details = get_string("resourcedetails_{$langstring}", 'inea',
 ...
 * @return void
 */
function inea_print_intro($resource, $cm, $course, $ignoresettings=false) {
    global $OUTPUT;

    $options = empty($resource->displayoptions) ? array() : unserialize($resource->displayoptions);

    $extraintro = inea_get_optional_details($resource, $cm);
    if ($extraintro) {
    ...
    if ($ignoresettings || !empty($options['printintro']) || $extraintro) {
        ...
        if ($gotintro || $extraintro) {
            ...
            if ($gotintro) {
                echo format_module_intro('inea', $resource, $cm->id);
            }
            echo $extraintro;
            echo $OUTPUT->box_end();
        }
 ...
 */
function inea_print_tobemigrated($resource, $cm, $course) {
    global $DB, $OUTPUT;

    $resource_old = $DB->get_record('inea_old', array('oldid'=>$resource->id));
    inea_print_header($resource, $cm, $course);
    inea_print_heading($resource, $cm, $course);
    inea_print_intro($resource, $cm, $course);
    echo $OUTPUT->notification(get_string('notmigrated', 'inea', $resource_old->type));
    echo $OUTPUT->footer();
    die;
}
 ...
 * @return void, does not return
 */
function inea_print_filenotfound($resource, $cm, $course) {
    global $DB, $OUTPUT;
    ...
    if ($resource_old) {
        ...
        echo $OUTPUT->notification(get_string('filenotfound', 'inea'));
    }
    echo $OUTPUT->footer();
    die;
}
 ...
 */
function inea_get_final_display_type($resource) {
    global $CFG, $PAGE;
 ...
 */
class inea_content_file_info extends file_info_stored {
...
function inea_set_mainfile($data) {
    global $DB;
    $fs = get_file_storage();
    ...
    if ($draftitemid) {
        ...
        file_save_draft_area_files($draftitemid, $context->id, 'mod_inea', 'content', 0, $options);
    }
    $files = $fs->get_area_files($context->id, 'mod_inea', 'content', 0, 'sortorder', false);
    if (count($files) == 1) {
        // only one file attached, set it as main file automatically
        $file = reset($files);
        file_set_sortorder($context->id, 'mod_inea', 'content', 0, $file->get_filepath(), $file->get_filename(), 1);
    }
}
