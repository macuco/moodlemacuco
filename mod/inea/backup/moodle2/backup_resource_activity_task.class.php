...
/**
 * Defines backup_inea_activity_task class
 *
 * @package     mod_inea
 * @category    backup
...
require_once($CFG->dirroot . '/mod/inea/backup/moodle2/backup_resource_stepslib.php');

/**
 * Provides the steps to perform one complete backup of the Resource instance
 */
class backup_inea_activity_task extends backup_activity_task {
    ...
    /**
     * Defines a backup step to store the instance data in the inea.xml file
     */
    protected function define_my_steps() {
        $this->add_step(new backup_resource_activity_structure_step('inea_structure', 'inea.xml'));
    }
    ...
    static public function encode_content_links($content) {
        ...
        // Link to the list of ineas.
        $search="/(".$base."\/mod\/inea\/index.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@RESOURCEINDEX*$2@$', $content);

        // Link to inea view by moduleid.
        $search = "/(".$base."\/mod\/inea\/view.php\?id\=)([0-9]+)/";
        // Link to inea view by recordid
        $search2 = "/(".$base."\/mod\/inea\/view.php\?r\=)([0-9]+)/";

        // Check whether there are contents in the resource old table.
        if (static::$resourceoldexists === null) {
            static::$resourceoldexists = $DB->record_exists('inea_old', array());
        }

        // If there are links to items in the inea_old table, rewrite them to be links to the correct URL
        // for their new module.
        if (static::$resourceoldexists) {
            // Match all of the ineas.
            $result = preg_match_all($search, $content, $matches, PREG_PATTERN_ORDER);

            // Course module ID ineas links.
            if ($result) {
                list($insql, $params) = $DB->get_in_or_equal($matches[2]);
                $oldrecs = $DB->get_records_select('inea_old', "cmid $insql", $params, '', 'cmid, newmodule');
            ...
            // Resource ID links.
            list($insql, $params) = $DB->get_in_or_equal($matches[2]);
            $oldrecs = $DB->get_records_select('inea_old', "oldid $insql", $params, '', 'oldid, cmid, newmodule');
