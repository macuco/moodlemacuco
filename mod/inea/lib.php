 ...
 */
function inea_supports($feature) {
    switch($feature) {
 ...
 * @return array
 */
function inea_get_extra_capabilities() {
 ...
 */
function inea_reset_userdata($data) {
    return array();
}
 ...
 * @return array
 */
function inea_get_view_actions() {
 ...
 * @return array
 */
function inea_get_post_actions() {
 ...
 */
function inea_add_instance($data, $mform) {
    global $CFG, $DB;
    ...
    inea_set_display_options($data);

    $data->id = $DB->insert_record('inea', $data);

    // we need to use context now, so we need to make sure all needed info is already in db
    $DB->set_field('course_modules', 'instance', $data->id, array('id'=>$cmid));
    inea_set_mainfile($data);

    $completiontimeexpected = !empty($data->completionexpected) ? $data->completionexpected : null;
    \core_completion\api::update_completion_date_event($cmid, 'inea', $data->id, $completiontimeexpected);
 ...
 * @return bool true
 */
function inea_update_instance($data, $mform) {
    global $CFG, $DB;
    require_once("$CFG->libdir/resourcelib.php");
    $data->timemodified = time();
    ...
    $DB->update_record('inea', $data);
    inea_set_mainfile($data);

    $completiontimeexpected = !empty($data->completionexpected) ? $data->completionexpected : null;
    \core_completion\api::update_completion_date_event($data->coursemodule, 'inea', $data->id, $completiontimeexpected);
 ...
 */
function inea_set_display_options($data) {
 ...
 * @return bool true
 */
function inea_delete_instance($id) {
    global $DB;

    if (!$resource = $DB->get_record('inea', array('id'=>$id))) {
        return false;
    }

    $cm = get_coursemodule_from_instance('inea', $id);
    \core_completion\api::update_completion_date_event($cm->id, 'inea', $id, null);

    // note: all context files are deleted automatically

    $DB->delete_records('inea', array('id'=>$resource->id));
 ...
 */
function inea_get_coursemodule_info($coursemodule) {
    global $CFG, $DB;
    ...
    if ($coursemodule->showdescription) {
        ...
        $info->content = format_module_intro('inea', $resource, $coursemodule->id, false);
    }
    ...
    $fs = get_file_storage();
    $files = $fs->get_area_files($context->id, 'mod_inea', 'content', 0, 'sortorder DESC, id ASC', false, 0, 0, 1);
    if (count($files) >= 1) {
    ...
    $display = inea_get_final_display_type($resource);

    if ($display == RESOURCELIB_DISPLAY_POPUP) {
        $fullurl = "$CFG->wwwroot/mod/inea/view.php?id=$coursemodule->id&amp;redirect=1";
    ...
    // add some file details as well to be used later by inea_get_optional_details() without retriving.
    // Do not store filedetails if this is a reference - they will still need to be retrieved every time.
    if (($filedetails = inea_get_file_details($resource, $coursemodule)) && empty($filedetails['isref'])) {
 ...
 */
function inea_cm_info_view(cm_info $cm) {
    global $CFG;
    require_once($CFG->dirroot . '/mod/inea/locallib.php');

    $resource = (object)array('displayoptions' => $cm->customdata);
    $details = inea_get_optional_details($resource, $cm);
    if ($details) {
 ...
 * @return array
 */
function inea_get_file_areas($course, $cm, $context) {
    $areas = array();
    $areas['content'] = get_string('resourcecontent', 'inea');
    return $areas;
}
 ...
 */
function inea_get_file_info($browser, $areas, $course, $cm, $context, $filearea, $itemid, $filepath, $filename) {
    global $CFG;
    ...
    if ($filearea === 'content') {
        ...
        if (!$storedfile = $fs->get_file($context->id, 'mod_inea', 'content', 0, $filepath, $filename)) {
            if ($filepath === '/' and $filename === '.') {
                $storedfile = new virtual_root_file($context->id, 'mod_inea', 'content', 0);
            } else {
                // not found
                return null;
            }
        ...
        require_once("$CFG->dirroot/mod/inea/locallib.php");
        return new inea_content_file_info($browser, $context, $storedfile, $urlbase, $areas[$filearea], true, true, true, false);
    }
 ...
 */
function inea_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    global $CFG, $DB;
    require_once("$CFG->libdir/resourcelib.php");
    ...
    if (!has_capability('mod/inea:view', $context)) {
        return false;
    }
   ...
   //     $fs2 = new file_storage_inea();
    ...
    do {
        if (!$file) {
            ...
            $resource = $DB->get_record('inea', array('id'=>$cm->instance), 'id, legacyfiles', MUST_EXIST);
            if ($resource->legacyfiles != RESOURCELIB_LEGACYFILES_ACTIVE) {
                return false;
            }
            if (!$file = resourcelib_try_file_migration('/'.$relativepath, $cm->id, $cm->course, 'mod_inea', 'content', 0)) {
                return false;
            }
            // file migrate - update flag
            $resource->legacyfileslast = time();
            $DB->update_record('inea', $resource);
        }
    } while (false);
 ...
 */
function inea_page_type_list($pagetype, $parentcontext, $currentcontext) {
 ...
 */
function inea_export_contents($cm, $baseurl) {
    global $CFG, $DB;
    $contents = array();
    ...
    $fs = get_file_storage();
    $files = $fs->get_area_files($context->id, 'mod_inea', 'content', 0, 'sortorder DESC, id ASC', false);
    ...
    foreach ($files as $fileinfo) {
        ...
        $file['fileurl']      = file_encode_url("$CFG->wwwroot/" . $baseurl, '/'.$context->id.'/mod_inea/content/'.$resource->revision.$fileinfo->get_filepath().$fileinfo->get_filename(), true);
 ...
 */
function inea_dndupload_register() {
    return array('files' => array(
                     array('extension' => '*', 'message' => get_string('dnduploadresource', 'mod_inea'))
                 ));
}
 ...
 */
function inea_dndupload_handle($uploadinfo) {
    ...
    return inea_add_instance($data, null);
}
 ...
 * @since Moodle 3.0
 */
function inea_view($resource, $course, $cm, $context) {
    ...
    $event->add_record_snapshot('inea', $resource);
    $event->trigger();
 ...
 * @since Moodle 3.2
 */
function inea_check_updates_since(cm_info $cm, $from, $filter = array()) {
 ...
 */
function mod_inea_core_calendar_provide_event_action(calendar_event $event,
                                                      \core_calendar\action_factory $factory) {
    $cm = get_fast_modinfo($event->courseid)->instances['inea'][$event->instance];
    ...
    return $factory->create_instance(
        get_string('view'),
        new \moodle_url('/mod/inea/view.php', ['id' => $cm->id]),
        1,
        true
    );
}
