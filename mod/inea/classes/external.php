 ...
 * @since      Moodle 3.0
 */
class mod_inea_external extends external_api {
    ...
    public static function view_resource($resourceid) {
        ...
        $resource = $DB->get_record('inea', array('id' => $params['resourceid']), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($resource, 'inea');
    ...
    public static function get_resources_by_courses($courseids = array()) {
        ...
        if (!empty($params['courseids'])) {
            ...
            $resources = get_all_instances_in_courses("inea", $courses);
            foreach ($resources as $resource) {
                ...
                list($resource->intro, $resource->introformat) = external_format_text($resource->intro,
                                                                $resource->introformat, $context->id, 'mod_inea', 'intro', null);
                $resource->introfiles = external_util::get_area_files($context->id, 'mod_inea', 'intro', false, false);
                $resource->contentfiles = external_util::get_area_files($context->id, 'mod_inea', 'content');
